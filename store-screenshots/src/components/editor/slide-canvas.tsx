...
// Black outline (縁取り) that renders in every browser:
// - 8 blurred offset shadows draw the outline ring themselves (no reliance on
//   -webkit-text-stroke), with blur smoothing the corners into rounded edges.
// - -webkit-text-stroke adds a crisp inner edge where supported.
// thicknessPx is the *visible* outline width.
function outlinedText(thicknessPx: number): React.CSSProperties {
  const w = Math.max(6, Math.round(thicknessPx));
  const o = Math.max(3, Math.round(w * 0.45));
  const b = Math.max(2, Math.round(w * 0.3));
  return {
    WebkitTextStroke: `${Math.max(4, Math.round(w * 0.6))}px #000`,
    textShadow: [
      `${o}px ${o}px ${b}px #000`,
      `-${o}px ${o}px ${b}px #000`,
      `${o}px -${o}px ${b}px #000`,
      ...
      `0 -${o}px ${b}px #000`,
      `0 0 ${b}px #000`,
      `0 ${Math.round(w * 0.4)}px ${Math.round(w * 0.9)}px rgba(0, 0, 0, 0.45)`,
    ].join(", "),
  };
}
