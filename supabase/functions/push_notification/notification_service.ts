 ...
 */ export class NotificationService {
   ...
   */ async handleRateLimit(userId) {
    const { data: lastLog, error: logFetchError } = await this.supabase.from("notification_logs").select("created_at").eq("user_id", userId).order("created_at", {
      ascending: false
    }).limit(1);
    if (logFetchError) {
    ...
    // 制限をクリアしたのでログを記録
    const { error: logInsertError } = await this.supabase.from("notification_logs").insert([
      {
        user_id: userId
      }
    ]);
   ...
   */ async sendMatchWaitingNotification(roomId, hostUserId) {
    ...
    const { data: targetUsers, error: usersError } = await this.supabase.from("users").select("id, fcm_token, notification_settings(is_notification_enabled, match_waiting_enabled)").neq("id", hostUserId).not("fcm_token", "is", null);
