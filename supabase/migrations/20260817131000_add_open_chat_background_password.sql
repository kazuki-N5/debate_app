...
-- 既にポリシーが存在する場合はエラーになるのを防ぐため、DO ブロックで囲むか手動での調整が必要な場合があります。
-- (Supabase環境によってはコンソールからの設定が推奨されます)
DO $$
BEGIN
    IF NOT EXISTS (
    ...
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies WHERE policyname = 'Auth Users Insert' AND tablename = 'objects' AND schemaname = 'storage'
    ) THEN
    ...
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies WHERE policyname = 'Auth Users Update' AND tablename = 'objects' AND schemaname = 'storage'
    ) THEN
