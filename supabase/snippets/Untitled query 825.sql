...
-- 3-2. ポストのいいね解除 → like_post 通知を削除
CREATE OR REPLACE FUNCTION public.remove_bbs_post_like_notification()
RETURNS TRIGGER LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
BEGIN
    DELETE FROM public.notifications
...
FOR EACH ROW EXECUTE FUNCTION public.remove_bbs_post_like_notification();

-- 3-3. コメントへのいいね → like_comment 通知
...
-- 3-4. コメントのいいね解除 → like_comment 通知を削除
CREATE OR REPLACE FUNCTION public.remove_bbs_comment_like_notification()
RETURNS TRIGGER LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
BEGIN
    DELETE FROM public.notifications
...
FOR EACH ROW EXECUTE FUNCTION public.remove_bbs_comment_like_notification();

-- 3-5. コメントへの返信 → reply_comment 通知
